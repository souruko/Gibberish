-- every timer will call this function when it starts
function CheckForWaitingTimerStart(key, data)

    for window_index,window_data in ipairs(savedata) do 

        if (window_data.load) then

            for timer_index, timer_data in ipairs(window_data[TRIGGER_TYPE.Timer_Start]) do 
                ...
                if timer_data.token ~= "" and timer_data.token ~= nil then
                    ...
                    if (string.find(key, timer_data.token)) then
                        ...
                        if timer_data.unique == true then
                            out_key = "7_"..window_index.."w_"..timer_index.."t"
                        else
                            out_key = "7_"..window_index.."w_"..timer_index.."t_"..start_time
                        end

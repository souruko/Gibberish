--[[
    CollectionLists child for TimerItems
]]


TimerControl = class( Turbine.UI.Control )

function TimerControl:Constructor(parent, width, window_index, timer_type, timer_index, data)
...
function TimerControl:Build()
    ...
    self.token_l:SetMarkupEnabled(true)
    self.token_l:SetFontStyle(Turbine.UI.FontStyle.Outline)
    local text = self.data.description
    if text == nil or text == "" then
        text = self.data.text
    end
    self.token_l:SetText("<rgb=#FFA640>["..savedata[self.window_index].name.."]</rgb>\n"..text.." ")
    ...
    self.description_l:SetText(self.data.token)
    ...
    self.MouseClick = function()
        self.parent:TimerControlClicked(self.data.id.."tid", self.data.icon, self.data.timer)
    end
